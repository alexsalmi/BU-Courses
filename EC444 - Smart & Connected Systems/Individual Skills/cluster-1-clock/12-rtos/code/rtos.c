...
bool countingUp = true; // variable to keep track of which direction we are currently counting
...
// Count up in binary every 1 second
static void countLED() {
  ...
  while(1) {
      ...
      // Increment or decrement counting variable based on global variable countingUp, loop back around if 16 or -1
      if(countingUp){
        num += 1;
        if(num == 16){
          num = 0;
...
// A button press to toggle from counting up to counting down or vice-versa
static void pushButton() {
  while(1){		// Forever
    if (flag == 1) {
      ...
      vTaskDelay(10 / portTICK_PERIOD_MS);		// Delay a bit
  }
}

// Read the current direction value and display to alphanumeric “up” or “down”
static void displayUporDown() {
  // Debug
  int ret;
  ...
  // Continuallyprints either up or down, based on the global variable countingUp
  while (1) {
    if(countingUp){
...
void app_main() {
    ...
    gpio_set_intr_type(BUTTON_GPIO, GPIO_PIN_INTR_POSEDGE);
    gpio_install_isr_service(ESP_INTR_FLAG_LEVEL3);	    // Install ISR
    gpio_isr_handler_add(BUTTON_GPIO, gpio_isr_handler, (void*) BUTTON_GPIO);	// Hook ISR to GPIO pin

    xTaskCreate(countLED,"countLED", 4096, NULL, 5, NULL);
    xTaskCreate(pushButton,"pushButton", 4096, NULL, 4, NULL);
    xTaskCreate(displayUporDown,"displayUporDown", 4096, NULL, 3, NULL);
}
