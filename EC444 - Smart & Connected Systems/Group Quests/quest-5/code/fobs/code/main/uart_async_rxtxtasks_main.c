...
// Main code for our fobs. When the button is pushed, the fob will transmit its ID/code via tx. It is also constantly searching for rx input. 
// It will recieve this rx input from the hub after the hub has verified the ID/code via the database. If the response from the hub is a success response, the LED on this hub will toggle.
...
static void IRAM_ATTR gpio_isr_handler(void* arg){
    ...
    const int len = strlen(pin);
    const int txBytes = uart_write_bytes(UART_NUM_1,pin,len);
