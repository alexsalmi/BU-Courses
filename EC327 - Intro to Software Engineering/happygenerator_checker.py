...
def happygen_tester(program_to_run):
    ...
    try:
        ...
        return False, "Your program exceeded 10 second maximum.",score
    ...
    if summary == 'fef63f1365b9b905c99289e845c88a4f6f05fcf33e9bb7479aa66c9e5e80fb24':
       score['standard_tests'] = 15
       res += " all OK\n"
    else:
        score['standard_tests'] = 0
        score['speed'] = 0
        res += " no match. Compare to answers.txt\n"       

    if score['standard_tests']:
        try:
            speed_info = T.stderr.splitlines()
            assert len(speed_info) == 7,"bad output in stderr"
            relative_speed = float(speed_info[1].split()[-1])
            overall_time = float(speed_info[-1].split()[-1])
            your_time = relative_speed * overall_time
            allocated_time = 0.5
            extra_time = your_time - allocated_time
            res += "allocated time is {:.2f}.\n".format(allocated_time)
            res += "your extra_time is {:.2f}\n".format(extra_time)
            res += "score formula is 30  -  20*extra_time (limited 0-30)\n"
            score['speed'] = max(0,min(30,30 -20*extra_time))
        except Exception as e:
            res += "problem with timing info: {}\nYour output:\n{}\n".format(str(e),T.stderr)
...
if __name__ == '__main__':
    PD = {'source_file':"happygenerator.cpp",'program_to_run':'happygenerator_tester','original_name':'happygenerator.cpp'}
